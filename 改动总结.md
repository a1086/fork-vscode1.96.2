...
## 25. 修复 Ports（转发端口）视图拖入编辑器区报错（2026-08-07）
...
## 26. 为 Secondary Side Bar（Auxiliary Bar）头部增加关闭按钮（2026-08-07）
...
- 新增 `CloseAuxiliaryBarAction`（`workbench.action.closeAuxiliaryBar`）：
  - 图标使用 `Codicon.close`。
  - 注册到 `MenuId.AuxiliaryBarHeader`，显示在辅助栏顶部全局 header 的工具栏中。
  - 执行时调用 `IWorkbenchLayoutService.setPartHidden(true, Parts.AUXILIARYBAR_PART)`，直接隐藏辅助栏。
  - 同时加入命令面板（`f1: true`），方便键盘/命令调用。

### 26.2 验证方式
1. `npx tsc --noEmit -p src/tsconfig.json` 通过（0 错误）。
2. 编译运行后，在 Secondary Side Bar 顶部会出现一个 `×` 关闭按钮；点击后辅助栏立即隐藏。
3. 命令面板中可搜索并执行 "Close Secondary Side Bar"。
