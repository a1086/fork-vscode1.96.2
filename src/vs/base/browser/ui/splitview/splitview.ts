 ...
 */
export type Sizing =
	DistributeSizing | SplitSizing | AutoSizing | InvisibleSizing;
...
> extends Disposable {
	...
	private onSashStart({ sash, start, alt }: ISashEvent): void {
		...
		const resetSashDragState = (start: number, alt: boolean) => {
			...
			if (!alt) {
				...
				const downIndexes =
					index + 1 < this.viewItems.length ? [index + 1] : [];
	...
	private resize(
		...
		// sash should be resized (`adjacentOnly`). This prevents non-adjacent
		// views (e.g. the third group in a 3-group row) from being pulled along.
		// For all other callers (initial layout / container resize / restore) we
		// fall back to the original behaviour of distributing the delta across
		// every view on each side so the serialized grid proportions are kept.
		const upIndexes = adjacentOnly ? [index] : range(index, -1);
		let downIndexes: number[];
		if (adjacentOnly) {
			downIndexes = index + 1 < this.viewItems.length ? [index + 1] : [];
		} else {
			downIndexes = range(index + 1, this.viewItems.length);
		}
