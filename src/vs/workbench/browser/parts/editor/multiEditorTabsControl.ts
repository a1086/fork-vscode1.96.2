		...
		disposables.add(
			new DragAndDropObserver(tab, {
				onDragStart: (e) => {
					...
					// Always write the dragged editors (including `ViewEditorInput`s) into the
					// editor transfer so that the editor area's own drop targets can reorder
					// them within a group or move them to another editor group.  `ViewEditorInput`
					// is a real `EditorInput`, so treating it like any other editor here is correct.
					...
					// Apply some datatransfer types to allow for dragging the element outside of the
					// application. Only *real* resources are written as resource transfers: a
					// `ViewEditorInput` resolves to a virtual `vscode-view/…` URI that
					// `ResourcesDropHandler` cannot open and would surface "Unable to resolve resource
					// vscode-view/…" errors. The editor area already recognizes the drag via the editor
					// transfer above (move/split/move across groups), and Panel/Sidebar/Auxiliary bar
					// targets via the composite transfer published by `publishViewDragData` below.
					const nonViewEditors = selectedEditors.filter(
						(e) => !(e instanceof ViewEditorInput),
					);
					...
					// A view hosted in the editor area must additionally publish its view id onto
					// the shared composite transfer, otherwise Panel/Sidebar/Auxiliary bar drop
					// targets do not recognize the drag and show no drop overlay.  This is independent
					// of the editor transfer above and is what enables dragging the view back out.
					this.publishViewDragData(editor);
