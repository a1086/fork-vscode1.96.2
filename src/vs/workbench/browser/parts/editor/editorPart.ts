...
export class EditorPart extends Part implements IEditorPart, IEditorGroupsView {
	...
	// Mirror of the editor/group drag transfers used by the tab controls.  We
	// read them here only to detect whether a drag originated *inside* the
	// editor area, so that an editor-hosted view being reordered/split is not
	// mistaken for an external "view dropped into the editor" operation.
	...
	private setupDragAndDropSupport(
		...
		this._register(
			CompositeDragAndDropObserver.INSTANCE.registerTarget(overlay, {
				onDragOver: (e) => {
					// A workbench view (single 'view') or a whole view container
					// ('composite') can be dropped into the editor area.  An editor
					// tab/group being reordered/split also carries a `view` payload
					// (published by `EditorTabsControl.publishViewDragData`); in that
					// case the editor's own `DropOverlay` must handle the drag, so we
					// bail out and let it through.
					...
					// When the cursor is near an edge, reveal the adjacent part (Panel /
					// Auxiliary Bar) so the view can be dropped there.  While in that
					// proximity zone we must NOT claim the drop for the editor, otherwise
					// the drag is swallowed by the editor overlay and the view can never
					// reach the (hidden) Panel/Auxiliary bar.  This edge logic used to
					// run only for editor-tab drags; views were short-circuited earlier
					// and therefore could never be dropped onto a collapsed aux/panel.
					...
					if (e.eventData.dataTransfer) {
						// Inside an edge-proximity zone: the drop is destined for the
						// adjacent part, not the editor.
				...
				onDrop: (e) => {
					...
					if (
						...
						// own `DropOverlay` handle it instead of re-importing the view here.
						if (
							this.editorTransfer.hasData(DraggedEditorIdentifier.prototype) ||
